...
First install is RECORD. There are no prompts, blocks, advisories, or output
...
logged. No finding means no log write. The log is local and is never included in
the model context.
...
The hook returns `{}` for clean, RECORD, disabled, or failed scans. This is
...
Payload reads are capped at 1 MiB, scanned text at 256 KiB per event, traversal
at 4,096 nodes, and scanning at 50 ms between rules. Oversized or incomplete
scans are recorded, not described as clean. The installed command has a 2 second
...
On the reference Windows host (2026-09-04 local date, Python 3.12.12, Claude Code
