...
def test_incomplete_evaluation_aborts_instead_of_reporting_zero(monkeypatch, state, phase, isolated):
    from agent_defs import bench

    scanner_name = "scan" if isolated else "scan_trusted"
    original = getattr(bench, scanner_name)
    incomplete = replace(ScanResult(partial_findings=(), rules_evaluated=1, rules_skipped_budget=0,
    ...
    monkeypatch.setattr(bench, scanner_name, scan)
    with pytest.raises(RuntimeError, match="incomplete benchmark"):
        measure([rule()], [corpus(1, 1)], isolated=isolated)
